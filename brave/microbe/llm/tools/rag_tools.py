...
async def rag_tools(emit_thinking, arguments: dict):
    # 这里写你自己的逻辑
    # data = f"模拟获取日志内容{biz_id}-{biz_type}"
    keywords = arguments["keywords"]
    title = f"正在搜索数据库 {keywords}"
    await emit_thinking("step2","running",{'title':title,'content': 'LLM started'})
    if "keywords" not in arguments:
        return "缺少参数 keywords"
    
    title = f"搜索数据库 {keywords} 完成"
    await emit_thinking("step2","running",{'title':title,'content': 'LLM started'})
